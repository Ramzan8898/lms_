                    ...
                    <div
                        ...
                        <div class="relative h-64 md:h-80 overflow-hidden group">
                            ...
                            <div class="absolute top-4 right-4">
                                <div
                                    class="px-4 py-2 bg-black/60 backdrop-blur-xl border border-yellow-500/30 rounded-full">
                                    <span
                            ...
                            @auth
                                ...
                                @else
                                    ...
                                    <button onclick="toggleWishlist({{ $course->id }})"
                                        ...
                                        <svg class="w-5 h-5 wishlist-icon" fill="{{ $inWishlist ? 'currentColor' : 'none' }}"
                                            stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linecap="round" stroke-width="2"
                                                d="M4.318 6.318a4.5 4.5 0 000 6.364L12 20.364l7.682-7.682a4.5 4.5 0 00-6.364-6.364L12 7.636l-1.318-1.318a4.5 4.5 0 00-6.364 0z" />
                                        </svg>
                                        <span
                                            class="wishlist-text">{{ $inWishlist ? 'In Wishlist' : 'Add to Wishlist' }}</span>
                                    </button>
                                    ...
                                    <script>
                                        ...
                                        function toggleWishlist(courseId) {
                                            ...
                                            button.innerHTML =
                                                '<svg class="w-5 h-5 animate-spin" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linecap="round" stroke-width="2" d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15"></path></svg> Processing...';
                            ...
                            <div
                                ...
                                <div class="divide-y divide-white/5 max-h-112.5 overflow-y-auto custom-scrollbar">
                                    @foreach ($course->lessons as $index => $lesson)
                                        <div class="p-4 hover:bg-white/5 transition-colors group">
                                            <div class="flex items-start gap-3">
                                                <div
                                                    class="w-8 h-8 rounded-full bg-yellow-500/20 flex items-center justify-center shrink-0">
                                                    <span
                                                        class="text-yellow-400 text-sm font-semibold">{{ $index + 1 }}</span>
                                                </div>
                                                <div class="flex-1">
                                                    <h4
                                                        class="text-white font-medium group-hover:text-yellow-400 transition-colors line-clamp-1">
                                                        {{ $lesson->title }}
                                                    </h4>
                                ...
                                @if ($course->lessons->count() > 5)
                                    <div class="p-4 border-t border-white/10">
                                        <a href="#"
                                            class="block w-full py-2 text-center text-yellow-400 hover:text-yellow-300 text-sm font-semibold transition-colors">
                                    ...
                                    <div class="p-4 hover:bg-white/5 transition-colors group">
                                        <div class="flex items-start gap-3">
                                            <div class="w-full flex items-center justify-center shrink-0">
                                                <span class="text-yellow-400 text-sm font-semibold">No Lessons
                                                    Yet...</span>
                                            </div>
                                        </div>
                                    </div>
                        ...
                        @if (isset($relatedCourses) && $relatedCourses->isNotEmpty())
                            <div
                                ...
                                @foreach ($relatedCourses as $related)
                                    <a href="{{ route('website.pages.show', $related->slug) }}"
                                        class="flex items-center gap-3 p-3 hover:bg-white/5 rounded-xl transition-colors group mb-2 last:mb-0">
                                        <img src="{{ $related->banner ? asset('storage/' . $related->banner) : 'https://images.unsplash.com/photo-1461749280684-dccba630e2f6?auto=format&fit=crop&w=800&q=80' }}"
                                            class="w-16 h-16 rounded-lg object-cover" alt="{{ $related->title }}">
                                        <div class="flex-1">
                                            <h4
                                                class="text-white text-sm font-semibold group-hover:text-yellow-400 transition-colors line-clamp-2">
                                                {{ $related->title }}
                                            </h4>
                                            <div class="flex items-center gap-2 mt-1">
                                                <span
                                                    class="text-yellow-400 text-xs font-bold">${{ number_format($related->price, 2) }}</span>
                                            </div>
                                        </div>
                                    </a>
                                @endforeach
