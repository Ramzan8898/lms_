...
@section('content')
    <div>
        ...
        <x-dashboard.breadcrumbs :items="[['label' => 'lesson', 'url' => route('admin.lessons.create')], ['label' => 'Create']]" />
    </div>

    <form action="{{ route('admin.lessons.store') }}" method="POST" enctype="multipart/form-data" class="flex flex-row gap-5">
        @csrf
        ...
        <div class="w-2/3 bg-linear-to-b from-[#1e1e1e] to-[#141414] border border-yellow-500/20 rounded-2xl shadow-xl p-6">
            ...
            <div class="mt-4">
                ...
                <select name="course_id" class="input cursor-pointer" required>
                    <option value="" class="bg-gray-900 cursor-pointer" disabled selected>Choose Course</option>
                    @foreach ($courses as $course)
                        <option value="{{ $course->id }}" class="bg-gray-900">{{ $course->title }}</option>
                    @endforeach
                </select>
            </div>
            <div class="mt-4">
                <label class="label">Lesson Type <span class="text-red-500">*</span></label>
                <select name="type" class="input cursor-pointer" required>
                    <option value="pdf" class="bg-gray-900" disabled selected>Select Lesson Type</option>
                    <option value="pdf" class="bg-gray-900">PDF</option>
                    <option value="video" class="bg-gray-900">Video</option>
                </select>
            </div>
        ...
        <div
            ...
            <div>
                <label class="label mb-2 block text-white">Upload File (PDF or Video) <span
                ...
                <!-- Upload / Preview Box -->
                <div id="uploadBox"
                    class="relative w-full h-34 rounded-2xl border-2 border-dashed border-gray-600 bg-linear-to-br from-gray-900 to-gray-800 flex items-center justify-center text-gray-400 text-center verflow-hidden cursor-pointer transition-all duration-300 over:border-yellow-500 hover:shadow-lg hover:shadow-yellow-500/10">
                    ...
                    <div id="previewContent" class="flex flex-col items-center gap-2">
                        ...
                        <p class="text-xs text-gray-500">PDF, TXT, MP4</p>
    ...
    <script>
        ...
        <div class="flex flex-col items-center gap-1">
            ...
            <span class="text-xs text-yellow-500">Ready to upload</span>
        </div>
    `;
        }
    </script>
