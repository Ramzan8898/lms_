    ...
    <form action="{{ route('admin.students.update', $student->id) }}" method="POST" enctype="multipart/form-data"
        ...
        <div class="w-2/3 bg-linear-to-b from-[#1e1e1e] to-[#141414] border border-yellow-500/20 rounded-2xl shadow-xl p-6">
            <div class="flex flex-row gap-5">
                <div class="w-full">
                    ...
                    <input type="text" name="name" placeholder="e.g: John Doe" class="input" required
                        value="{{ $student->user->name }}">
                </div>
                <div class="w-full">
                    <label for="phone" class="label">Phone Number <span class="text-red-500">*</span></label>
                    <input type="text" name="phone" placeholder="e.g: 0000 0000000" class="input" required
                        value="{{ $student->phone }}">
                </div>
            </div>
            <div class="flex flex-row gap-5 mt-4">
                <div class="w-full">
                    ...
                    <input type="email" name="email" placeholder="e.g: johndoe@gmail.com" class="input" required
            ...
            <div class="flex flex-row gap-5 mt-4">
                <div class="w-full">
                    ...
                    <select name="status" class="input" required>
                        <option value="active" {{ $student->status == 'active' ? 'selected' : '' }} class="bg-gray-900">
                            Active</option>
                        <option value="inactive" {{ $student->status == 'inactive' ? 'selected' : '' }} class="bg-gray-900">
                            Inactive</option>
                    </select>
                </div>
            </div>
        ...
        <div class="w-1/3 bg-linear-to-b from-[#1e1e1e] to-[#141414] border border-yellow-500/20 rounded-2xl shadow-xl p-6">
            <div class="flex flex-col w-full gap-6">
                ...
                <div class="w-full">
                    ...
                    <div id="bannerPreview"
                        ...
                        <!-- Image Preview -->
                        <img id="previewImage" src="{{ $student->avatar ? asset($student->avatar) : '' }}"
                            class="absolute inset-0 w-full h-full object-cover transition-opacity duration-300 rounded-2xl {{ $student->avatar ? '' : 'hidden' }}">
            ...
            <div class="w-full mt-4">
                ...
                <textarea name="address" rows="4" class="input">{{ $student->address }}</textarea>
            </div>
