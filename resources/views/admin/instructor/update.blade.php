...
@section('content')
    <div>
        ...
        <x-dashboard.breadcrumbs :items="[['label' => 'Instructors', 'url' => route('admin.instructors')], ['label' => 'Edit']]" />
    </div>
    ...
    <form action="{{ route('admin.instructor.update', $instructor->id) }}" method="POST" enctype="multipart/form-data"
        ...
        <div class="w-2/3 bg-linear-to-b from-[#1e1e1e] to-[#141414] border border-yellow-500/20 rounded-2xl shadow-xl p-6">
            <div class="flex flex-row gap-5">
                <div class="w-full">
                    ...
                    <input type="text" name="name" class="input" placeholder="e.g: John Doe"
                        value="{{ $instructor->user->name }}" required>
                </div>
                <div class="w-full">
            ...
            <div class="flex flex-row gap-5 mt-4">
                <div class="w-full">
                    ...
                    <input type="text" name="expertise" placeholder="e.g:Web Development, Android, Math..."
                        class="input" value="{{ $instructor->expertise }}">
                </div>
            ...
            <div class="flex flex-col gap-4 mt-4">
                <div>
                    ...
                    <input type="text" name="qualification" class="input" value="{{ $instructor->qualification }}"
                ...
                <div>
                    ...
                    <select name="status" class="input">
                        <option value="active" {{ $instructor->status == 'active' ? 'selected' : '' }} class="bg-gray-900">
                            Active</option>
                        <option value="inactive" {{ $instructor->status == 'inactive' ? 'selected' : '' }}
                            class="bg-gray-900">
                            Inactive</option>
                    </select>
                </div>
        ...
        <div class="w-1/3 bg-linear-to-b from-[#1e1e1e] to-[#141414] border border-yellow-500/20 rounded-2xl shadow-xl p-6">
            <div class="flex flex-col w-full gap-6">
                ...
                <div class="w-full">
                    ...
                    <div id="bannerPreview"
                        ...
                        <!-- Image Preview -->
                        <img id="previewImage" src="{{ $instructor->avatar ? asset($instructor->avatar) : '' }}"
                            class="absolute inset-0 w-full h-full object-cover transition-opacity duration-300 rounded-2xl {{ $instructor->avatar ? '' : 'hidden' }}">
            ...
            <div class="mt-4">
                ...
                <textarea name="bio" rows="5" class="input" placeholder="Enter Short Bio About Instructor !">{{ $instructor->bio }}</textarea>
            </div>
        </div>
    </form>
    <script>
