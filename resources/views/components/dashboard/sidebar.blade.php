...
<aside
    class="w-80 bg-linear-to-b from-[#0a0a0a] via-[#0f0f0f] to-black border-r border-yellow-600/20 text-white hidden md:flex flex-col min-h-screen overflow-y-auto no-scrollbar">
    ...
    <div class="flex flex-col items-center py-6 border-b border-yellow-600/20 group">
        ...
        <div class="relative mb-4 transition-all duration-500 group-hover:scale-105">
            ...
            <div
                class="absolute -inset-3 bg-linear-to-r from-yellow-500/40 to-orange-500/40 rounded-full blur-xl opacity-0 group-hover:opacity-60 transition duration-500">
            </div>

            <a href="#">
                <img src="{{ $setting->logo }}"
                    class="w-54 relative z-10 drop-shadow-[0_0_15px_rgba(255,215,0,0.3)] group-hover:drop-shadow-[0_0_25px_rgba(255,215,0,0.6)] transition-all duration-500"></a>
        </div>
        ...
        @if ($userRole === 'admin')
           ...
           {{ request()->routeIs('admin.dashboard') ? 'text-yellow-400 bg-white/5' : 'text-gray-400 hover:text-yellow-400 hover:bg-white/5' }}">
                ...
                {{ request()->routeIs('admin.dashboard') ? 'text-yellow-400' : 'text-gray-500 group-hover:text-yellow-400' }}"
                    ...
                    <path stroke-linecap="round" stroke-width="2" d="M3 12l2-2 7-7 7 7 2 2M5 10v10h14V10" />
                </svg>
    ...
    {{ request()->routeIs('admin.queries') ? 'text-yellow-400 bg-white/5' : 'text-gray-400 hover:text-yellow-400 hover:bg-white/5' }}">
        ...
        @if ($userRole === 'instructor')
           ...
           {{ request()->routeIs('admin.courses') ? 'text-yellow-400 bg-white/5' : 'text-gray-400 hover:text-yellow-400 hover:bg-white/5' }}">
                ...
                @php
                    $totalCourses = App\Models\Course::where(
                        'instructor_id',
                        Auth::user()->instructor->id ?? 0,
                    )->count();
                @endphp
                <span class="px-2 py-0.5 bg-white/10 rounded-full text-xs text-gray-400">
                    {{ $totalCourses }}
...
$totalLessons = App\Models\Lesson::whereIn(
    'course_id',
    App\Models\Course::where('instructor_id', $instructorId)->pluck('id'),
        ...
        <div class="flex justify-center">
            <a href="{{ route('welcome') }}"
                ...
                <!-- Icons -->
                <svg class="w-6 h-6 text-yellow-400 group-hover:rotate-12 transition-transform duration-500"
                    fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linecap="round" stroke-width="2"
                        d="M3 12l2-2 7-7 7 7 2 2M5 10v10h14V10"></path>
                </svg>

                <span class="relative">Visit Website</span>

                <svg class="w-5 h-5 text-yellow-400 group-hover:translate-x-2 transition-transform duration-300"
                    fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linecap="round" stroke-width="2"
                ...
                <!-- Animated border -->
                <span
                    class="absolute bottom-0 left-0 w-full h-0.5 bg-linear-to-r from-yellow-500 to-orange-500 transform scale-x-0 group-hover:scale-x-100 transition-transform duration-500 origin-left"></span>
            </a>
        </div>
    </nav>
    ...
    <div class="p-6 border-t border-yellow-600/20">
        ...
        <form method="POST" action="{{ route('auth.logout') }}">
            ...
            <button type="submit"
                ...
                <span
                    class="absolute inset-0 bg-linear-to-r from-yellow-500/10 to-orange-500/10 opacity-0 group-hover:opacity-100 transition duration-500"></span>

                <span
                    class="relative flex items-center justify-center gap-3 text-yellow-400 font-semibold tracking-wider text-sm
                             group-hover:text-yellow-300 transition">

                    <svg class="w-5 h-5 group-hover:rotate-12 transition-transform duration-500" fill="none"
                        stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-width="2" d="M17 16l4-4-4-4M7 12h14" />
                    </svg>
