...
<header
    class="w-full bg-(--black)/95 shadow-2xl py-4 border-b border-(--primary)/30 fixed z-999 top-0 backdrop-blur-md bg-opacity-95"
    style="backdrop-filter: blur(12px); -webkit-backdrop-filter: blur(12px);">

    {{-- Animated linear line at top --}}
    <div
        class="absolute top-0 left-0 w-full h-0.5 bg-linear-to-r from-transparent via-(--primary) to-transparent animate-shimmer">
    </div>
        ...
        <div class="flex flex-row items-center gap-6 lg:gap-8">
            ...
            <nav class="hidden md:flex items-center gap-8 text-gray-200 font-medium">
                <a href="{{ route('welcome') }}"
                    ...
                    <span
                        class="absolute bottom-0 left-0 w-full h-0.5 bg-(--primary) transform transition-transform duration-300 origin-left 
                                 {{ $currentRoute === 'welcome' ? 'scale-x-100' : 'scale-x-0 group-hover:scale-x-100' }}"></span>
                    <span
                        class="absolute inset-0  opacity-0 group-hover:opacity-100 transition-opacity duration-300 z-0 rounded
            ...
            @auth
                @php
                    $user = Auth::user();
                    $routeName = $user->hasRole('admin') ? 'admin.dashboard' : 'student.courses.show';
          ...
          {{ $currentRoute === $routeName ? 'ring-2 ring-(--primary) ring-offset-2 ring-offset-(--black)' : '' }}"
                    style="background-size: 200% auto;" onmouseover="this.style.backgroundPosition='right center'"
                    onmouseout="this.style.backgroundPosition='left center'">

                    <span
                        class="absolute inset-0 rounded-xl border-2 border-transparent group-hover:border-amber-300/30 
                 animate-pulse-slow"></span>

                    <span class="relative">
                        Dashboard
                        <span
                            class="absolute -bottom-1 left-0 w-full h-0.5 bg-white/30 scale-x-0 group-hover:scale-x-100 
                     transition-transform duration-300 origin-left
                     {{ $currentRoute === $routeName ? 'scale-x-100' : '' }}"></span>
                    </span>

                    <i
                        class="fas fa-chevron-right text-xs opacity-70 group-hover:translate-x-1.5 group-hover:opacity-100 
            ...
            @else
                ...
                <a href="{{ route('login') }}"
                    ...
                    <span class="relative z-10 flex items-center gap-2">
                        <i
                            class="fas fa-arrow-right-to-bracket text-sm opacity-80 group-hover:opacity-100 
                              group-hover:translate-x-1 transition-all duration-300"></i>
                        <span class="relative">
                            Sign In
                            <span
                                class="absolute -bottom-1 left-0 w-full h-0.5 bg-(--dark) scale-x-0 group-hover:scale-x-100 
                                     transition-transform duration-300 origin-left
                    ...
                    {{-- Glow particles --}}
                    <span
                        class="absolute w-20 h-20 bg-(--primary)/30 rounded-full blur-3xl opacity-0 
            ...
            <button
                class="block md:hidden text-white text-2xl focus:outline-none hover:text-(--primary) 
                           transition-all duration-300 hover:rotate-90 hover:scale-110 relative group"
                aria-label="Toggle menu" x-data @click="$dispatch('toggle-mobile-menu')">
                <i class="fas fa-bars"></i>
                <span
                    class="absolute inset-0 bg-(--primary)/20 rounded-full blur-xl opacity-0 
    ...
    {{-- Bottom shimmer line --}}
    <div class="absolute bottom-0 left-0 w-full h-px bg-linear-to-r from-transparent via-(--primary)/50 to-transparent">
    </div>
</header>
